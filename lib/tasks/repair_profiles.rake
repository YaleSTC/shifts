namespace :db do
  def make_empty_profile_entries(department_name)
    ...
    all_fields_master = UserProfileField.all.select{|upf| upf.department_id == department.id}
    users.each do |user|
      all_fields = all_fields_master
      ...
      puts "#{user.name}, entries added: #{missing_fields.size}"
    end
  rescue
    puts "#{department_name} is not a valid department name. Please enter a valid one."    
  end
