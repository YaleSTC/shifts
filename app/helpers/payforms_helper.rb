module PayformsHelper
  ...
  def payform_add_button
    unless @payform.approved
      link_to '<span>New Payform Item</span>', new_payform_payform_item_path(@payform) + "?height=500&width=600", :title => "Create New Payform Item", :class => "thickbox button", :onclick => "this.blur();"
    end
  end
