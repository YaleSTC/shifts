class DataEntry < ActiveRecord::Base
  ...
  scope :on_day, lambda {|day| { :conditions => ["#{:created_at.to_sql_column} > #{day.beginning_of_day.to_sql} and #{:created_at.to_sql_column} < #{day.end_of_day.to_sql}"]}}
  scope :between_days, lambda {|first, last| { :conditions => ["#{:created_at.to_sql_column} > #{first.beginning_of_day.to_sql} and #{:created_at.to_sql_column} < #{last.end_of_day.to_sql}"]}}
  scope :for_data_object, lambda {|data_object| { :conditions => {:data_object_id => data_object.id }}}
