class Calendar < ActiveRecord::Base
  ...
  def deactivate
    self.active = false
    TimeSlot.update_all("active = '#{false.to_sqlness}'", "calendar_id = '#{self.id}'")
    Shift.update_all("active = '#{false.to_sqlness}'", "calendar_id = '#{self.id}'")
    self.save
  end
  ...
  def activate
    ...
    if conflicts.empty?
      TimeSlot.update_all("active = '#{true.to_sqlness}'", "calendar_id = '#{self.id}' AND start > '#{Time.now.utc.to_s(:sql)}'")
      Shift.update_all("active = '#{true.to_sqlness}'", "calendar_id = '#{self.id}' AND start > '#{Time.now.utc.to_s(:sql)}'")
      self.save
      return false
    else
      return conflicts
