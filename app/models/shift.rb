...
#TODO: remove all to_sql calls except where needed for booleans
  ...
  #Prepares an array of shift arrays. Each inner array coresponds to all active shifts in a given location. 
  #The whole array is passed along to be rendered in a partial as a collection. This allows us to display 
  #active shifts grouped by location
  def self.signed_in_shifts_by_location_in_department(current_department)
    @signed_in_shifts = []
    signed_in_shifts_array = Shift.signed_in(current_department).group_by(&:location_id).sort #.sort converts the hash generated by group_by to an array of arrays
    signed_in_shifts_array.each do |element|
      @signed_in_shifts << element[1] #element[0] is the location_id for this array of shifts, we don't want that passed to the view
    end
    @signed_in_shifts
  end
