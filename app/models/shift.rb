...
#  before_validation :adjust_end_time_if_in_early_morning, :if => Proc.new{|shift| shift.scheduled?}
  ...
  def stats_display_missed
     "#{start.to_s(:am_pm)} - #{self.end.to_s(:am_pm)}, #{user.name}, #{location.name} <a href='mailto:#{user.email}?cc=decal@yale.edu&subject=Missed+#{location.short_name}+Shift+&body=Hi%20#{user.first_name}%2C%0A%0AYou%20missed%20your%20#{location.short_name}%20shift%20yesterday.%20What%20happened%3F%0A%0A%0ADeCaL'>[email]</a>".html_safe
  end

  def stats_display_late
     "#{self.report.id}: #{start.to_s(:am_pm)} - #{self.end.to_s(:am_pm)}, #{user.name}, #{location.name} <a href='mailto:#{user.email}?cc=decal@yale.edu&subject=Late+#{location.short_name}+Shift+&body=Hi%20#{user.first_name}%2C%0A%0AYou%20were%20late%20to%20your%20#{location.short_name}%20shift%20yesterday.%20What%20happened%3F%0A%0A%0ADeCaL'>[email]</a>".html_safe
  end

  def stats_display_left_early
     "#{self.report.id}: #{start.to_s(:am_pm)} - #{self.end.to_s(:am_pm)}, #{user.name}, #{location.name} <a href='mailto:#{user.email}?cc=decal@yale.edu&subject=Left+Early+From+#{location.short_name}+Shift+&body=Hi%20#{user.first_name}%2C%0A%0AYou%20left%20your%20#{location.short_name}%20shift%20early%20yesterday.%20What%20happened%3F%0A%0A%0ADeCaL'>[email]</a>".html_safe
  end
