...
#  named_scope :all_including_inactive, :conditions => {}
  ...
  def self.make_future(end_date, cal_id, r_e_id, days, loc_id, start_time, end_time, user_id, department_id, active, wipe)
    ...
    days.each do |day|
      ...
      while seed_end_time <= end_date
        ...
        inner_test.push "(user_id = \"#{user_id}\" AND active = \"true\" AND department_id = \"#{department_id}\" AND start <= \"#{seed_end_time.utc.to_s(:sql)}\" AND end >= \"#{seed_start_time.utc.to_s(:sql)}\")"
  ...
  def self.check_for_conflicts(shifts)
    if shifts.empty?
      ...
      Shift.find(:all, :conditions => [shifts.collect{|s| "(user_id = \"#{s.user_id}\" AND active = \"true\" AND department_id = \"#{s.department_id}\" AND start <= \"#{s.end.utc.to_s(:sql)}\" AND end >= \"#{s.start.utc.to_s(:sql)}\")"}.join(" OR ")]).collect{|t| "The shift for "+t.to_message_name+" conflicts. Use wipe to fix."}.join(",")
    end
  end
