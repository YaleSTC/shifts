...
#TODO: remove all to_sql calls except where needed for booleans
  ...
  named_scope :stats_sent, :conditions => {:stats_sent => false}
  named_scope :stats_unsent, :condition => {:stats_unsent => true}
  named_scope :missed, 
        :joins => :report, 
        :conditions =>["#{:end.to_sql_column} < ?",Time.now]
  named_scope :late,
    :joins => :report,
  ...
  def stats_display
    "#{user.name}, #{location.name}, #{start.to_s(:am_pm)} - #{self.end.to_s(:am_pm)}, #{start.to_s(:just_date)}"
  end    
