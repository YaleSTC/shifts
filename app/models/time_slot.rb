...
#  named_scope :all_including_inactive
    ...
    loc_ids.each do |loc_id|
      days.each do |day|
        ...
        while seed_end_time <= end_date
          ...
          inner_test.push "(location_id = '#{loc_id}' AND active = '#{true.to_sql}' AND start <= '#{seed_end_time.utc.to_s(:sql)}' AND end >= '#{seed_start_time.utc.to_s(:sql)}')"
  ...
  def self.check_for_conflicts(time_slots)
    if time_slots.empty?
      ...
      TimeSlot.find(:all, :conditions => [time_slots.collect{|t| "(location_id = '#{t.location_id}' AND active = '#{true.to_sql}' AND start <= '#{t.end.utc.to_s(:sql)}' AND end >= '#{t.start.utc.to_s(:sql)}')"}.join(" OR ")]).collect{|t| "The timeslot "+t.to_message_name+" conflicts. Use wipe to fix."}.join(",")
    end
  end
