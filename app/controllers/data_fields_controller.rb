class DataFieldsController < ApplicationController
  ...
  def create
    ...
    if @data_field.save
      ...
      redirect_to (params[:add_another] ? new_data_type_data_field_path(params[:data_type_id]) : ( @data_field.data_type.data_objects.empty? ? new_data_type_data_object_path(params[:data_type_id]) : data_type_path(params[:data_type_id]) ))
    else
      render :action => 'new'
    end
  end
